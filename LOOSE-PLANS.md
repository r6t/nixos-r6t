...
- Profiles are self-contained and direct-import needed options.nix/config.nix.
...
- homeManagerModules._, nixosModules.default, packages, checks, and nixosConfigurations._ stay stable.
...
- [x] Remove from all hosts:
- [x] ../../modules/default.nix
- [x] direct upstream home-manager imports where covered by r6t-home-core
- [x] direct upstream nix-flatpak imports where covered by kde-workstation
- [x] This is the next best big swing because it separates internal composition from compatibility API.
...
- Split and direct-import profile-owned modules that still rely on mine.\*.enable.
- Incus pass:
- incus
- incus-log-collector
...
- use path:/home/r6t/git/nixos-r6t#... while new imported files are untracked
  Decision
  Phase 1 is complete. I recommend next implementation step: Phase 2 Incus pass, splitting and direct-importing Incus-adjacent profile-owned modules while preserving container build attrs, runtime profiles, instance_map.json, and Tailscale/DNS semantics.
